...
class ComplianceDefinition(BaseModel):
    ...
    name: str = Field(
        ..., description="Human-readable name", min_length=1, max_length=100
    )

    description: str = Field(
        ..., description="Detailed description", min_length=10, max_length=1000
    )

    threshold: float = Field(
        ..., ge=0.0, le=1.0, description="Pass threshold between 0.0 and 1.0"
    )
    ...
    def validate_prompt_template(cls, v: str) -> str:
        ...
        if missing:
            ...
            raise ValueError(
                f"Prompt template must contain placeholders: {', '.join(required_placeholders)}"
            )
